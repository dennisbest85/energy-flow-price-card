...
- **Flow line style:** `wire_style` (`dashed` by default, or `neon`) controls how the four flow lines look, independent of the visual layout — `dashed` is the original moving-dash look, `neon` is a solid line with a soft glow.
...
- **Stijl flow-lijnen:** `wire_style` (`dashed` standaard, of `neon`) bepaalt hoe de vier flow-lijnen eruitzien, los van de visuele weergave — `dashed` is de originele bewegende-streepjes-stijl, `neon` is een effen lijn met een zachte gloed.
