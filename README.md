...
reduce_effects: false        # true = drop the neon glow filter / text-shadow, for weaker-GPU devices (editor shows this under visual layout)
wire_style: dashed           # "dashed" (flowing dashes, current look) | "neon" (green base line, glowing pulse sweeps through)
...
- **Usage tab:** a fourth chart tab showing the home's power (W) over the last hour, split by source — solar, net battery (discharge minus charge) and grid (import/export). It reuses the `solar_power`, `battery_charge_power`, `battery_discharge_power` and `grid_power` entities already configured for the flow diagram — no extra entities to fill in. The tab appears automatically once any of those are set.
...
- **Visual layout:** `use_visual_layout` (off by default) replaces the abstract wire diagram's background with an actual house photo — solar, grid, battery and car lines are re-routed to specific spots on the picture (solar panels, battery box, car on the driveway) instead of a shared abstract square, and run in clean horizontal/vertical segments (rather than a curve) so they stay legible over the photo, always drawn in front of it. All the value labels and icons stay in exactly the same positions, and the card's height doesn't change — the photo is scaled to fit. The small house-icon badge in the center is hidden in this mode (the photo already shows the house); its power value stays.
...
- **Verbruik-tabblad:** een vierde grafiek-tabblad dat het vermogen (W) van het huis over het afgelopen uur toont, uitgesplitst per bron — solar, netto accu (ontladen min laden) en net (import/export). Hergebruikt de entiteiten `solar_power`, `battery_charge_power`, `battery_discharge_power` en `grid_power` die al voor het flow-diagram zijn ingesteld — geen extra entiteiten nodig. Het tabblad verschijnt automatisch zodra één daarvan is ingevuld.
...
- **Visuele weergave:** `use_visual_layout` (standaard uit) vervangt de achtergrond van het abstracte lijnendiagram door een echte huisfoto — de lijnen van solar, net, accu en auto worden herrouteerd naar specifieke plekken op de foto (zonnepanelen, accukastje, auto op de oprit) in plaats van naar een gedeeld abstract vierkant, en lopen in nette horizontale/verticale stukken (in plaats van een curve) zodat ze goed leesbaar blijven boven op de foto, altijd op de voorgrond. Alle waarde-labels en iconen blijven exact op dezelfde plek staan, en de hoogte van de kaart verandert niet — de foto wordt verkleind om te passen. Het kleine huisje-icoontje in het midden verdwijnt in deze modus (de foto toont het huis al); de vermogenswaarde blijft gewoon staan.
