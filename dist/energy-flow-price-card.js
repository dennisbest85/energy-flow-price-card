...
class EnergyFlowPriceCard extends i {
  ...
  _renderFlow() {
    ...
    // smooth curve. `reversed` swaps which end is the path start (house vs icon).
    const elbow = (ax, ay, bx, by) => `M${ax},${ay} L${bx},${ay} L${bx},${by}`;
    const wireD = (iconX, iconY, ep, curveCtrlX, reversed) => visual
      ? (reversed ? elbow(ep.x, ep.y, iconX, iconY) : elbow(iconX, iconY, ep.x, ep.y))
    ...
    return b`
      <div class="flow">
        ...
        <svg class="wires" viewBox="0 0 720 190" preserveAspectRatio="none">
          ...
          ${wSolar.show ? w`<path class="${liveClass(wSolar)}" style="${liveStyle(wSolar, c.color_solar)}" d="${wireD(70, 52, EP.solar, 220, false)}"></path>` : A}

          <path class="wire" d="${wireD(650, 52, EP.grid, 500, false)}"></path>
          ${wGrid.show ? w`<path class="${liveClass(wGrid)}" style="${liveStyle(wGrid, c.color_grid)}" d="${wireD(650, 52, EP.grid, 500, gridPow < 0)}"></path>` : A}

          <path class="wire" d="${wireD(70, 138, EP.battery, 220, false)}"></path>
          ${wBatt.show ? w`<path class="${liveClass(wBatt)}" style="${liveStyle(wBatt, c.color_battery)}" d="${wireD(70, 138, EP.battery, 220, v.charge && v.charge > 5)}"></path>` : A}

          <path class="wire" d="${wireD(650, 138, EP.car, 500, false)}"></path>
          ${wCar.show ? w`<path class="${liveClass(wCar)}" style="${liveStyle(wCar, c.color_car)}" d="${wireD(650, 138, EP.car, 500, true)}"></path>` : A}
        </svg>
